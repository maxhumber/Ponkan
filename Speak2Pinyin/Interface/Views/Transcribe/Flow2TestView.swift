...
struct FlowLayout<Element: Identifiable & Equatable, Cell: View>: View {
    ...
    var measureStart: Int {
        sizes.keys.sorted().last ?? 0
    }
    
    var measureEnd: Int {
        min(measureStart + 20, lastVisibleIndex)
    }
    ...
    var body: some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            ...
            ScrollView(showsIndicators: false) {
                ...
                .onPreferenceChange(MaxVisibleViewPreference.self) { value in
...
struct Flow2TestView_Previews: PreviewProvider {
    static var previews: some View {
        Flow2TestView()
    }
    
    struct Flow2TestView: View {
        @State var items: [Item] = (1...1_000).map { "Item \($0) " + (Bool.random() ? "\n" : "")  + String(repeating: "x", count: Int.random(in: 0...10)) }.map { Item(value: $0) }
